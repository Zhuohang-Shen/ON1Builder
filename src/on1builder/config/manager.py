...
class ConfigurationManager:
    ...
    def initialize(
        ...
        if self._config is not None and not force_reload:
            logger.debug(
                "Configuration already loaded. Use force_reload=True to reload."
            )
            return
    ...
    def _validate_chain_configurations(self, config: Dict[str, Any]) -> None:
        ...
        for key, value in rpc_urls.items():
            try:
                normalized_rpc_urls[int(key)] = value
            except (TypeError, ValueError):
                logger.warning(f"Ignoring RPC URL with non-integer key: {key}")

        missing_rpcs = []
        for chain_id in chains:
            if chain_id not in normalized_rpc_urls or not normalized_rpc_urls[chain_id]:
    ...
    def _validate_api_configurations(self, config: Dict[str, Any]) -> None:
        ...
        if not any(price_apis):
            ...
            logger.warning(
                "No price API keys configured - using free tier with rate limits"
            )
    ...
    def _log_configuration_summary(self) -> None:
        ...
        notifications_data = config_dict.get("notifications", {}) or {}
        channels = notifications_data.get("channels") or []

        summary = {
            "chains_configured": len(config_dict.get("chains", [])),
            "wallet_configured": bool(config_dict.get("wallet_address")),
            "api_keys_configured": sum(
                1 for key in config_dict.get("api", {}).values() if key
            ),
            "notifications_enabled": bool(channels),
            "debug_mode": config_dict.get("debug", False),
            "config_file": (
                str(self._config_file_path) if self._config_file_path else "unknown"
            ),
        }
