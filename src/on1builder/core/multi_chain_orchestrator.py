...
class MultiChainOrchestrator:
    ...
    async def start(self):
        ...
        for chain_id in self.workers.keys():
            ...
            self.balance_managers[chain_id] = BalanceManager(web3, wallet_address)
        ...
        while self.is_running:
            try:
                ...
                if opportunities:
                    ...
                    logger.info(
                        f"Found {len(opportunities)} opportunities, {len(viable_opportunities)} viable after scoring."
                    )
        ...
        self._opportunity_history.append(
            {
                ...
                and buy_result.get("success", False),
                "sell_success": not isinstance(sell_result, Exception)
                and sell_result.get("success", False),
            }
        )
            ...
            for sell_chain in chains[i + 1 :]:
                ...
                for buy_chain_id, sell_chain_id, buy_info, sell_info in [
                    ...
                    if spread > min_spread_required:
                        ...
                        logger.info(
                            f"Cross-chain arbitrage opportunity: {token_symbol} "
    ...
    async def _calculate_optimal_trade_size(
        ...
        liquidity_limit = (
            Decimal("1000") * opportunity["liquidity_score"]
        ...
        try:
            ...
            for dex_name, router_address in routers.items():
                try:
                    ...
                    logger.warning(
                        f"Failed to query {dex_name} liquidity for {token_symbol}: {e}"
                    )
                    continue
    ...
    async def _generate_opportunity_analysis(self):
        ...
        try:
            recent_opportunities = [
                opp
                for opp in self._opportunity_history
            ...
            best_token = (
                max(token_performance.items(), key=lambda x: x[1]["profit"])[0]
    ...
    async def _query_pool_liquidity(
        ...
        try:
            ...
            if token_a and token_b:
                ...
                if any(
                    addr.lower() in ["weth", "usdc", "usdt"]
                    for addr in [token_a.lower(), token_b.lower()]
                ):
            ...
            logger.warning(f"Failed to query pool liquidity: {e}")
            return Decimal("0")
