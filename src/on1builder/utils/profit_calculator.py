    ...
    async def _parse_token_movements(self, logs: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for log in logs:
            try:
                ...
                if (
                    len(log.topics) > 0
                    and log.topics[0].hex() == self._event_signatures["Transfer"]
                ):
                ...
                # Check for FlashLoan events
                    ...
                    len(log.topics) > 0
                    and log.topics[0].hex() == self._event_signatures["FlashLoan"]
                ):
    ...
    async def _parse_swap_log(self, log: Dict) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Failed to decode swap data: {decode_error}")
                return None
        ...
        try:
            ...
            return {
                ...
                "profit_margin_percent": float(
                    (net_profit_usd / max(total_outflow_usd, Decimal("1"))) * 100
                ),
            }
    ...
    async def _get_strategy_specific_analysis(
        ...
        try:
            if strategy_type == "arbitrage":
                ...
                analysis["mev_profit_extracted"] = any(
                    change > 0 for change in net_changes.values()
                )
                analysis["victim_transactions"] = len(
                    [m for m in movements if m.get("type") == "swap"]
                )

            elif strategy_type == "liquidation":
                # Liquidations should show asset acquisition at discount
                analysis["liquidation_bonus_estimated"] = any(
                    change > 0 for change in net_changes.values()
                )
        ...
        try:
            ...
            if token_info and "decimals" in token_info:
                ...
                try:
                    ...
                    logger.debug(
                        f"Failed to call decimals() for {token_address}: {contract_error}"
                    )
    ...
    async def calculate_flash_loan_profit(self, tx_hash: str) -> Dict[str, Any]:
        ...
        try:
            ...
            flash_loan_analysis = {
                ...
                "flash_loan_protocols": list(
                    set(m.get("protocol_address") for m in flash_loan_events)
                ),
